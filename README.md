...
进入插件配置，开启 **使用后台订阅列表管理推送**，在 **群聊与私聊订阅列表** 中添加推送目标：
...
多个机器人连接可分别配置。后台模式下只有列表中启用的目标会推送，聊天命令不能修改订阅。
禁用、删除条目或清空列表都会停止对应推送；已有去重和待发状态保留，再启用可能继续发送待发内容。
旧命令订阅保留但暂停，关闭后台管理开关后恢复旧命令管理方式（包括已建立的后台订阅记录）。
